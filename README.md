# Simple Introduction (LDumberger)

SPA = Frontend Application
API = Backend Application (written in JS)

1. Install node_modules in Folder SPA and API with `npm ci`
2. Configure SPA (Frontend) in `authConfig.js` by replacing any occourence of `APPLICATION_ID`, `TENANT_ID` and check `const loginRequest`-scope
3. Configure API (Backend) in `authConfig.js` by replacing any occourence `APPLICATION_ID`, `TENANT_ID`
4. Run both SAP and API with `npm start`

Original Source of this Repo: https://github.com/Azure-Samples/ms-identity-javascript-tutorial/blob/main/3-Authorization-II/1-call-api/README.md
